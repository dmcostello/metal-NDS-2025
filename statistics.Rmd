...
#Build dataset
tunedata <- cbind(resp=as.factor(predclass[,6]),predclass[,c(2,10:108)])
...
#Build Fe dataset
rfFedata <- cbind(Fe_TF=as.factor(predclass[,'Fe_TF']),predclass[,c(2,10:108)])
...
#Run rf model and evaluate performance (24% misclass)
set.seed(71)
rfFe <- randomForest(Fe_TF~.,
    data=Fetrain,mtry=90,maxnodes=4,ntree=700)
rfFe

#Test data 75% accuracy (3 of 8 misclassified)
...
#save(rfFe,file="./models/rfFe.Rdata")
```

```{r nitrogen random forest model}
#Build N dataset
rfNdata <- cbind(N_TF=as.factor(predclass[,'N_TF']),predclass[,c(2,10:108)])
...
#Run rf model and evaluate performance (30% misclass)
set.seed(9)
rfN <- randomForest(N_TF~.,
    data=Ntrain,mtry=10,maxnodes=5,ntree=1100)
rfN
...
#save(rfN,file="./models/rfN.Rdata")
```

```{r phosphorus random forest model}
#Build Fe dataset
rfPdata <- cbind(P_TF=as.factor(predclass[,'P_TF']),predclass[,c(2,10:108)])
...
#Run rf model and evaluate performance (45% misclass)
set.seed(7)
rfP <- randomForest(P_TF~.,
    data=Ptrain,mtry=45,maxnodes=3,ntree=1500)
rfP

#Test data 75% accuracy (4 of 8 misclassified)
...
#save(rfP,file="./models/rfP.Rdata")
```

```{r zinc random forest model}
#Build Zn dataset
rfZndata <- cbind(Zn_TF=as.factor(predclass[,'Zn_TF']),predclass[,c(2,10:108)])
...
#Run rf model and evaluate performance (21% misclass)
set.seed(30)
rfZn <- randomForest(Zn_TF~.,
    data=Zntrain,mtry=65,maxnodes=5,ntree=600)
rfZn
...
#save(rfZn,file="./models/rfZn.Rdata")
```
