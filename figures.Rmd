...
#Impute missing WQ data
impWQ <- mice(WQ_NDS,m=1,maxit=50,meth='pmm',seed=111,print=F)
impWQ$imp$Mo_ugL

#New dataset with imputed data
WQ_NDS_comp <- complete(impWQ)

#Calculate ratios
WQ_NDS_comp$TNTP <- (WQ_NDS_comp$TN_ugL/14)/(WQ_NDS_comp$TP_ugL/31)
WQ_NDS_comp$NO3SRP <- (WQ_NDS_comp$NO3_ugL/14)/(WQ_NDS_comp$SRP_ugL/31)
WQ_NDS_comp$TNFe <- (WQ_NDS_comp$TN_ugL/14)/(WQ_NDS_comp$Fe_ugL/55.845)
WQ_NDS_comp$TPFe <- (WQ_NDS_comp$TP_ugL/31)/(WQ_NDS_comp$Fe_ugL/55.845)

#Make merged dataset - 41 streams, 99 predictors
land <- merge(LULC,NDSsoil[,c(2,4:24)],by="Stream")
landWQ <- merge(land,WQ_NDS_comp,by.x="Stream",by.y="Site")
...
PDP_Ndep <- partialPlot(rfFe,predclass,INORGNWETDEP2008WS,"Yes",n.pt=100)
PDP_P <- partialPlot(rfFe,predclass,SRP_ugL,"Yes",log="x",n.pt=100)

load("./models/rfZn.Rdata")

#Individual partial plot for Zn
PDP_PZn <- partialPlot(rfZn,predclass,SRP_ugL,"Yes",log="x") 
...
FeAr <- RGBrast("./raw data/A_Fe.tif")
ZnAr <- RGBrast("./raw data/A_Zn.tif")
```
...
#Similar function but for paneling without y-axis labels
RRplot2 <- function(RRdata,xmin=-1,xmax=3){ 
par(pty="m",mar=c(4,0,2,2))

stripchart(RRdata[,3:13],pch=1,cex=0,las=1,xlim=c(xmin,xmax),ylim=c(0.5,11.5),xlab="ln response ratio",group.names = "")
rect(-0.326,0,0.326,15,col="grey90",border=NA)
abline(v=0)

#Single element treatments
for(i in 1:4){points(RRdata[,2+i],11-i+jitter(rep(1,41),factor=5),cex=1.5,
                     col=ifelse(RRdata[,2+i]>0.326,"royalblue","gray30"))}
...
for(i in c(1:5,7)){points(RRdata[,6+i],7-i+jitter(rep(1,41),factor=5),cex=1.5,
                     col=ifelse(RRdata[,20+i] %in% intopt,"goldenrod2","gray30")
                     #,pch=ifelse(RRdata[,20+i] %in% intopt,16,1)
                     )}
  #Only did NxFe for 2022
with(RRchl[RRdata$Site %in% good22,],
             points(NFe,1+jitter(rep(1,14),factor=5),cex=1.5,
                     col=ifelse(NxFecl %in% intopt,"goldenrod2","gray30")
                    #,pch=ifelse(NxFecl %in% intopt,16,1)
                    ))

text(xmax,7.4,"CO-LIMIT",adj=1)
for(i in 21:27){text(xmax,(7:1)[i-20],paste(sum(RRdata[,i] %in% intopt,na.rm=T),
                                  "/",sum(!is.na(RRdata[,i])),sep=""),adj=1)}
}
...
#pdf("Figure_1.pdf",width=10,height=4.5)
plot_grid(
...
pdf("Figure_2.pdf",height=6,width=4.5)
...
pdf(file="Figure_3.pdf",height=4.5,width=10)
par(mfrow=c(1,4),oma=c(0,5,0,0))
RRplot2(RRTot,xmax=4)
mtext("a. Total biomass",3,cex=0.7)
#mtext("a",3,adj = -0.2.cex=1.5)
mtext(RRnames,side=2,las=2,at=seq(11,1),cex=0.7,line=1)
RRplot2(RRdiatom,xmax=4)
mtext("b. Diatom",3,cex=0.7)
RRplot2(RRcyano,xmax=4)
mtext("c. Cyanobacteria",3,cex=0.7)
RRplot2(RRgreen,xmin=-4,xmax=6)
mtext("d. Chlorophyte",3,cex=0.7)
dev.off()
```

```{r Figure 4 - Iron RF model}
#pdf("Figure_4.pdf",width=5,height=5)
layout(matrix(c(2,1,1,1, 2,1,1,1, 2,1,1,1, 0,3,3,3),4,4))

#Main scatterplot
par(mai=c(0.6,0.6,0,0))
plot(INORGNWETDEP2008WS~SRP_ugL,data=predclass,log="x",col=c("black","orangered")[factor(Fe_TF)],xlim=c(1,100),ylim=c(2,8),las=1,pch=c(1,16)[factor(Fe_TF)],
...
plot(PDP_P$y~PDP_P$x,type="l",log="x",xlim=c(1,100),xaxt="n",las=1,
     xlab="",ylim=c(-0.2,0.4),ylab="logit(Fe)",cex.lab=1.2)
text(1,0.35,"b",cex=2)
logaxis(0,3,1,labels=F)
text(13,c(0.12,0),c("Probable","Fe limitation"),col="orangered")
abline(v=5.57,lty=2)

#Partial dependency for N dep
par(mai=c(0.6,0.2,0,0.2))
plot(PDP_Ndep$x~PDP_Ndep$y,type="l",ylim=c(2,8),yaxt="n",las=1,
     ylab="",xlim=c(-0.5,0.6),xlab="logit(Fe)",cex.lab=1.2)
text(-0.4,8,"c",cex=2)
...
#Partial dependency for DRP
pdf("Figure_5.pdf",width=5,height=3)
par(mar=c(4.5,4.5,0.5,0.5))
plot(PDP_PZn$y~PDP_PZn$x,type="l",xlim=c(1,100),xaxt="n",las=1,
     xlab="DRP concentration (µg/L)",ylim=c(-0.7,0.2),
     ylab="logit(Zn)",cex.lab=1.2,log="x")
logaxis(0,2,1)

#Overlay stripchart
stripchart(predclass[predclass$Zn_TF=="No",'SRP_ugL'],col="black",
           pch=1,at=-0.2,add=T,method = "jitter",cex=1.2,jitter = 0.2)
stripchart(predclass[predclass$Zn_TF=="Yes",'SRP_ugL'],col="purple3",
           pch=16,at=-0.2,add=T,method = "jitter",cex=1.2,jitter=0.2 )
